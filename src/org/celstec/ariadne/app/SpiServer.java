...
public class SpiServer implements SpiSwordServer {
	...
	public DepositResponse doDeposit(Deposit deposit)
		...
		if (deposit.getContentType() != null
				&& deposit.getContentType().contains("application/atom+xml"))
	...
	public void putMetadata(Deposit deposit) {
		...
		Element id = atom.getChild("id", Namespace.getNamespace("atom",
		...
		if (contextPath.contains("deposit/metadataForMid")) {
			resourceId = contextPath.substring(contextPath
					.indexOf("deposit/metadataForMid") + 23);
		}
		if (contextPath.contains("deposit/metadataForRid")) {
			metadataId = contextPath.substring(contextPath
					.indexOf("deposit/metadataForRid") + 23);
		}
		sg.submitMetadata(authorizationToken, metadataId, resourceId,
				getMetadataFromDeposit(atom), metadataSchemaId, null);

	}
	
	public void putResource(Deposit deposit){
		String authorizationToken = deposit.getUsername()
		+ deposit.getPassword();
		String resourceId = "";
		String contextPath = ((SpiDeposit) deposit).getContextPath();
		if (contextPath.contains("deposit/resource")) {
			resourceId = contextPath.substring(contextPath
					.indexOf("deposit/resource") + 17);
		}
		...
		sg.submitResource(authorizationToken, resourceId, resource,
				packageType, contentType, collection, filename);
	}

	public ServiceDocument doServiceDocument(ServiceDocumentRequest arg0)
			throws SWORDAuthenticationException, SWORDException {
	...
	private DepositResponse doMetadataDeposit(Deposit deposit) {
		...
		Element id = atom.getChild("id", Namespace.getNamespace("atom",
				"http://www.w3.org/2005/Atom"));
		Element titleElement = atom.getChild("title", Namespace.getNamespace(
		...
		String metadataId = sg.submitMetadata(authorizationToken,
				metadataIdentifier, resourceId, getMetadataFromDeposit(atom),
		...
		if (!resourceId.equals(""))
			mediaEditLinkEntry = PropertiesManager.getProperty("app.baseURL")
					+ "/deposit/resource/" + resourceId;
		String editLinkEntry = PropertiesManager.getProperty("app.baseURL")
				+ "/deposit/metadataForMid/" + metadataId;

		dr.setEntry(createReturnEntry(title, resourceId, mediaEditLinkEntry,
				editLinkEntry));
		dr
				.setLocation(PropertiesManager.getProperty("app.baseURL")
						+ "/services/oai?verb=GetRecord&metadataPrefix=oai_lom&identifier="
						+ metadataIdentifier);
		return dr;
	}
	...
	private DepositResponse doResourceDeposit(Deposit deposit) {
		...
		String filename = deposit.getFilename();
		SpiGateway sg = new SpiGateway();
		sg.submitResource(authorizationToken, resourceIdentifier, resource,
				packageType, contentType, collection, filename);
		DepositResponse dr = new DepositResponse(Deposit.CREATED);
		dr.setLocation("http://localhost:80/todo");
		String mediaEditLinkEntry = PropertiesManager
				.getProperty("app.baseURL")
				+ "/deposit/resource/" + resourceIdentifier;
		String editLinkEntry = PropertiesManager.getProperty("app.baseURL")
				+ "/deposit/metadataForRid/" + resourceIdentifier;
		dr.setEntry(createReturnEntry(deposit.getSlug(), resourceIdentifier,
				mediaEditLinkEntry, editLinkEntry));

		return dr;
	}

	private SWORDEntry createReturnEntry(String title, String identifier,
			String mediaEditLinkEntry, String editLinkEntry) {
	...
	public AtomDocumentResponse doAtomDocument(AtomDocumentRequest arg0)
	...
	private Element getMetadataFromDeposit(Element atom) {
		if (atom == null)
			return null;
		Element spi = atom.getChild("metadata", Namespace.getNamespace("spi",
				"http://www.cenorm.be/xsd/SPI"));
		if (spi == null)
			return null;
		Element lom = spi.getChild("lom", Namespace.getNamespace("lom",
	...
	private String getTitleFromLom(Element atom) {
		...
		Element general = lom.getChild("lom", Namespace.getNamespace("lom",
				"http://ltsc.ieee.org/xsd/LOM"));
		if (general == null)
			return null;
		Element identifier = lom.getChild("identifier", Namespace.getNamespace(
				"lom", "http://ltsc.ieee.org/xsd/LOM"));
		if (identifier == null)
			return null;
		Element entry = lom.getChild("entry", Namespace.getNamespace("lom",
				"http://ltsc.ieee.org/xsd/LOM"));
		if (entry == null)
			return null;
