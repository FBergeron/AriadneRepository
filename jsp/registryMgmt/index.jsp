...
<%  String logSystemDir = PropertiesManager.getInstance().getProperty("repository.log4j.directory");
	...
	Authenticator.setDefault(new Authenticator() {
		protected PasswordAuthentication getPasswordAuthentication() {
			return new PasswordAuthentication ("aspect", "tomcat4aspect".toCharArray());
		}
	});
	try { 
		URL url = new URL("http://monet:3781/aspect-harvester/install/ariadneV4.properties");
		URLConnection connection = url.openConnection();
		properties.load(connection.getInputStream()); 
		System.out.println(properties.getProperty("aplitic_xtec_cat.active"));
	} catch (IOException e) { 
...
<%
	...
	<%								}
			    ...
			    if (properties.getProperty(title+".active")!=null){
					...
					out.println("<input type=\"submit\" name=\"delete\" value=\"Delete from the harvester >>\" /></form></form></td></tr>");
			    }else{
