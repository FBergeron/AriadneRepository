...
<title>Online MACE Admin Tool</title>
<%
...
if (metadata != null){
	...
	if(getOrPublishString.equalsIgnoreCase("publish")){
		try{
	PublishMetadata.getMACEPublish().publishMetadata(metadata);
	result = "Publishing successful. You can check the : <a href=\"" + PropertiesManager.getProperty("admin.metadata.url") + id + "\" target=\"_blank\">metadata</a> and the <a href=\"" + PropertiesManager.getProperty("admin.resource.url") + id + "\" target=\"_blank\">resource</a>";
		}catch(Exception e){
	error = e.getMessage();
		}
	}
